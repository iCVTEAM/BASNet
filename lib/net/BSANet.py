...
class BSANet(nn.Module):
    def __init__(self, cfg):
        ...
        self.semantic_trans = nn.Sequential(
            nn.Conv2d(indim, asppdim, 3, 1,
                      padding=1, bias=True),
            SynchronizedBatchNorm2d(asppdim, momentum=cfg.TRAIN_BN_MOM),
            nn.ReLU(inplace=True),
        )

        self.semantic_output = nn.Conv2d(indim, cfg.MODEL_SEMANTIC_CLASSES, 1, 1, padding=0)
    ...
    def forward(self, x, mode):
        ...
        # ori implementation uncomment this line to re-implement
        # feature_att = F.relu_(torch.mul(self.sigmoid(fc_att).expand_as(feature_cat),feature_cat))

        # use dense attention to get a little higher performance boost +0.30% miou for large dataset
        feature_att = F.relu_(torch.mul(self.sigmoid(feature_semantic), feature_cat))

        feature_final = torch.cat([feature_cat, feature_att], 1)

        ins_r = self.semantic_output(feature_semantic)

        feature_final = self.finaloutput(feature_final)
