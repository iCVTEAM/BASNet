...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        if 'instance' in sample.keys():
            ins = sample['instance']
            ins = cv2.resize(ins, dsize=(new_w, new_h), interpolation=self.seg_interpolation)
            if self.fix:
                ins = cv2.copyMakeBorder(ins, top, bottom, left, right, cv2.BORDER_CONSTANT, value=[0])
            sample['instance'] = ins
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        image, segmentation,edge,instance = sample['image'], sample['segmentation'],sample['edge'],sample['instance']
        ...
        instance = instance[top: top + new_h,
               left: left + new_w]
...
class RandomFlip(object):
    ...
    def __call__(self, sample):
        ...
        if np.random.rand() < self.flip_t:
            ...
            edge_flip = np.flip(edge, axis=1)
            instance_flip = np.flip(instance, axis=1)
            sample['edge'] = edge_flip
            sample['instance'] = instance_flip
...
class RandomRotation(object):
    ...
    def __call__(self, sample):
        ...
        new_instance = cv2.warpAffine(instance, m, (col,row), flags=self.seg_interpolation, borderValue=0)

        new_edge = cv2.warpAffine(edge, m, (col,row), flags=self.seg_interpolation, borderValue=0)


        sample['image'] = new_image
        sample['segmentation'] = new_segmentation

        sample['instance'] = new_instance
...
class RandomScale(object):
    ...
    def __call__(self, sample):
        ...
        ins = cv2.resize(instance, None, fx=rand_scale, fy=rand_scale, interpolation=self.seg_interpolation)
        edg = cv2.resize(edge, None, fx=rand_scale, fy=rand_scale, interpolation=self.seg_interpolation)
    ...
    def __call__(self, sample):
        ...
        for key in key_list:
            if 'image' in key:
                ...
                sample['instance'] = torch.from_numpy(instance.astype(np.float32))
            elif 'instance_onehot' == key:
                onehot = sample['instance_onehot'].transpose((2,0,1))
                sample['instance_onehot'] = torch.from_numpy(onehot.astype(np.float32))

            elif 'edge' == key:
                edge = sample['edge']
                sample['edge'] = torch.from_numpy(edge.astype(np.float32))
            elif 'edge_onehot' == key:
                onehot = sample['edge_onehot'].transpose((2, 0, 1))
                sample['edge_onehot'] = torch.from_numpy(onehot.astype(np.float32))
